...
def estimate_normal(p):
    ...
    U, s, Vh = svd( MP  )
    ## Asserts data fits well into a plane. Last singular value should
    ## be 'zero' for a plane. Of course you should be way more
    ## generous if you are fitting curved surfaces such as in this
    ## project.
    print s
    assert s[-1] < s[-2] * 2e-2

    ## Solution is taken from the last right singular vector. Its last
    ## value is also supposed to be close to zero due to subtracting
    ## mean form vectors, but that's not important to check. The three
    ## frist values give the normal direction, and we then normalize
    ## it (again).
    sol = Vh[-1,:-1]
